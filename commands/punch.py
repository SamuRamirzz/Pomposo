...
class PunchCog(commands.Cog):
    def __init__(self, bot):
        ...
        # Términos de búsqueda variados para más diversidad
        self.search_terms = [
            "cat fight",
            "cat punch",
            "cats fighting",
    ...
    async def get_gif_from_tenor(self, search_term: str) -> str:
        ...
        try:
            params = {
                ...
                "limit": 30,  # Aumentado para más variedad
    ...
    async def get_punch_gif(self) -> str:
        """
        Obtiene un GIF aleatorio de gatos peleando desde múltiples fuentes.
        Alterna entre Tenor y Giphy para mayor variedad.
        """
    ...
    def _get_fallback_gif(self) -> str:
        ...
        fallback_gifs = [
            "https://media.tenor.com/VsJB0EshLPQAAAAM/cat-fight.gif",
            "https://media.tenor.com/9kJPqLqLqLQAAAAM/cats-fighting.gif",
            "https://media1.giphy.com/media/hFXwY4lER3oBO/giphy.gif",
            "https://media.giphy.com/media/O5xChSjqUIxsk/giphy.gif"
        ]
        return random.choice(fallback_gifs)

    @commands.command(name="punch")
    async def punch_command(self, ctx, *, user_query: str):
        """
        Le da un puñetazo a un usuario (de forma divertida).

        Uso: ¿punch <nombre/mención/ID>
        Ejemplos:
            ¿punch @Usuario
            ¿punch 123456789
            ¿punch juan (busca usuarios similares a "juan")
        """
        try:
            ...
            if target_member.id == ctx.author.id:
                return await ctx.send("Pinche masoquista")

            if target_member.id == self.bot.user.id:
                return await ctx.send("Fuera....")

            # Obtener GIF de Tenor
            gif_url = await self.get_punch_gif()

            # Crear embed
            embed = discord.Embed(
                title="TOMAAAAAAAAAAAAAAAAAAAAAAAAAA",
                description=f"**{ctx.author.display_name}**le pego a **{target_member.display_name}...**!",
    ...
    async def punch_slash(self, interaction: discord.Interaction, usuario: discord.Member):
        """Comando slash para punch."""
        ...
        embed = discord.Embed(
            title="TOMAAAAAAAAAAAAAAAAAAAAAAAAAA",
            description=f"**{interaction.user.display_name}** le pego a **{usuario.display_name}...**!",
            color=discord.Color.red()
        )
        embed.set_image(url=gif_url)
        embed.set_footer(
